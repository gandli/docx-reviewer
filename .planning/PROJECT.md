...
- [ ] 用户可在设置页统一管理提示词偏好、主题色和本地模型，并在本地浏览器内持久化
...
- 检索与抽取链路进一步明确为：`pdfjs-dist` 负责保留阅读顺序的 PDF 文本抽取；`.docx` 优先走 `DecompressionStream + XML parse`；切块采用按句边界滑动窗口并保持 `50% overlap`；嵌入模型固定为 `all-MiniLM-L6-v2` 并运行在 Worker 中；检索先走 Voy cosine search，再做 MMR 重排。
...
| 设置页统一承载提示词偏好、主题色和本地模型 | 这些都属于工作台级偏好，集中管理更稳定，也更便于本地持久化 | — Pending |
| 文档抽取链路优先减少不必要依赖 | `pdfjs-dist` 与浏览器原生 `DecompressionStream` 更利于长期维护与离线运行 | — Pending |
